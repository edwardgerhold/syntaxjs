...
function get_alloc_size(bytes) {
    return get_overhead_size() + bytes;
}
function get_overhead_size() {
    return TYPE_SIZE + SIZE_SIZE + FREELIST_PTR_SIZE;
}
...
function getIndex(startIndex, field_size, field_num) {
    var start = startIndex+get_overhead_size();
...
console.log("heap debug msg: overhead size each allocation for information is: " + get_overhead_size());
